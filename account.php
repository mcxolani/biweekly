...
<<<<<<< HEAD
	  ...
	  <?php
			...
			$id = $core->getuserfield('id');
				$query = "select * FROM orders WHERE customer_id='$id' order by time";
				$run_query = $db->query($query);
						while($row = mysql_fetch_assoc($run_query)){
						?>
							<tr>
					<td><a href='customer_orders.php?order_num=<?php echo $row['order_num']; ?>'><?php echo $row['order_num']; ?></a></td>
					<td><?php echo $row['time']; ?></td>
					<td><?php echo 'R'.number_format($row['price'],2); ?></td>
					<td>

						<?php if($row['order_status']=='1'){echo "approved";}elseif ($row['order_status'] =='2') {
							echo "rejected";
						}else{ echo "Proccessing";} ; ?></td>
							</tr>
							<?php
							}
